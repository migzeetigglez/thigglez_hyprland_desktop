...
def main():
    ...
    artist = run_playerctl(["metadata", "--format", "{{artist}}"])
    title = run_playerctl(["metadata", "--format", "{{title}}"])
    album = run_playerctl(["metadata", "--format", "{{album}}"])
    art_url = run_playerctl(["metadata", "--format", "{{mpris:artUrl}}"])

    length_us = run_playerctl(["metadata", "--format", "{{mpris:length}}"])
    ...
    payload = {
        "text": text,
        "class": status_lower,
        "tooltip": f"{album or 'Unknown Album'}"
    }
